...
def get_projects():
    rows = database_api.get_projects(
...
def get_project(project_id):
    row = database_api.get_project_by_id(project_id)
    if row is None:
...
def get_project_grants(project_id):
    if not database_api.project_exists(project_id):
        return jsonify({"error": "Project not found"}), 404
    rows = database_api.get_project_grants(project_id)
    return jsonify(rows_to_list(rows)), 200
...
def get_project_publications(project_id):
    if not database_api.project_exists(project_id):
        return jsonify({"error": "Project not found"}), 404
    rows = database_api.get_project_publications(project_id)
    return jsonify(rows_to_list(rows)), 200
...
def get_project_staff(project_id):
    if not database_api.project_exists(project_id):
        return jsonify({"error": "Project not found"}), 404
    rows = database_api.get_project_staff(project_id)
    return jsonify(rows_to_list(rows)), 200
